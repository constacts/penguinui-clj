...
(defn layout
  ...
  [:div {:x-data (str "{ sidebarIsOpen: false }")
   ...
   [:nav {:class "fixed left-0 z-30 flex h-svh w-60 shrink-0 flex-col border-r border-neutral-300 bg-neutral-50 p-4 transition-transform duration-300 md:w-64 md:translate-x-0 md:relative dark:border-neutral-700 dark:bg-neutral-900"
          :x-bind:class "sidebarIsOpen ? 'translate-x-0' : '-translate-x-60'"
          :aria-label "sidebar navigation"}
    [:div {:class (str "flex flex-col gap-2 overflow-y-auto pb-6 h-full")}
